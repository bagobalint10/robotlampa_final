 ...
 */ 
 ...
 void timer_init_tmp(void)
 {
	 ...
	 TCCR1B |= (1 << WGM13) | (1 << WGM12) | (0 << CS12) | (1 << CS11) | (0 << CS10); // prescaler = 1

	 TIMSK1 |= (1 << TOIE1);

	 OCR1A = TIM_COMPARE;  // kitöltés 
	 ICR1 = time_levels[0];// kezdõ freki 
 ...
 void motor_main(void)
 {
	...
	if((!motor_enable) && (input_pos != current_pos) && ( tim_count > 20) && ( tim_count < 10000)) //áll a motor	+ menni kéne
	{
		if(input_pos>current_pos)	direction = 1;		//pozitiv
		else 	 direction = 0;							//negatív
		motor_enable = 1;
		//tim_refresh_complete = 0;

		PORT_Write(&MOTOR_DIR_PORT,MOTOR_DIR_PIN,direction);

		TCCR1A |= (1 << COM1A1) | (0 << COM1A0); // PWM kimenet engedélyezése

	}else if((!current_level) && (!motor_enable) && ( tim_count > 20) && ( tim_count < 10000) )		 //&& ( tim_count > 20)
	{	  
		//reset kimenet
