 ...
 */ 
 ...
 static void motor_update_timer(void)
 {
	...
	else if(motor_enable && (!direction))
	{
		...
		current_pos--;	   
		ICR1 = time_levels[current_level];			// timer komparálási ovf érték --> frek frisstés
	} 
 ...
 void motor_1_init(void)
 {
	...
	ICR1 = time_levels[0];		// alap frekvencia beállítása 
 ...
 void motor_1_main(uint16_t dmx_pos_1)
 {
	...
	tim_state = ((uint16_t)(TCNT1H << 8)) | ((uint16_t) TCNT1L);	// kiolvasni a timer értékét 
	...
	{	
		...
		PORT_Write(&MOTOR_DIR_PORT,MOTOR_DIR_PIN,direction);		// direction pin állítás
